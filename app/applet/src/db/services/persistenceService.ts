  ...
  return await db.transaction(async (tx) => {
    ...
    if (analysisData.riesgos && Array.isArray(analysisData.riesgos)) {
      ...
      if (risksToInsert.length > 0) {
        // En SQLite (si aplica), podemos insertar en batch. 
        // Si hay límite, partimos en chunks de 100, pero Drizzle lo maneja bien normalmente.
        const batchSize = 100;
    ...
    // 3. Si se proporciona un taskId, actualizar la tabla tasks dentro de la transacción
